...
contract CoreStabilityFeeTreasury {
    // --- Auth ---
    ...
    **/
    modifier isAuthorized {
        require(authorizedAccounts[msg.sender] == 1, "CoreStabilityFeeTreasury/account-not-authorized");
        _;
    }
    ...
    modifier accountNotTreasury(address account) {
        require(account != address(this), "CoreStabilityFeeTreasury/account-cannot-be-treasury");
        _;
    }
    ...
    function addition(uint256 x, uint256 y) internal pure returns (uint256 z) {
        z = x + y;
        require(z >= x, "CoreStabilityFeeTreasury/add-uint-uint-overflow");
    }
    function addition(int256 x, int256 y) internal pure returns (int256 z) {
        z = x + y;
        if (y <= 0) require(z <= x, "CoreStabilityFeeTreasury/add-int-int-underflow");
        if (y  > 0) require(z > x, "CoreStabilityFeeTreasury/add-int-int-overflow");
    }
    function subtract(uint256 x, uint256 y) internal pure returns (uint256 z) {
        require((z = x - y) <= x, "CoreStabilityFeeTreasury/sub-uint-uint-underflow");
    }
    function subtract(int256 x, int256 y) internal pure returns (int256 z) {
        z = x - y;
        require(y <= 0 || z <= x, "CoreStabilityFeeTreasury/sub-int-int-underflow");
        require(y >= 0 || z >= x, "CoreStabilityFeeTreasury/sub-int-int-overflow");
    }
    function multiply(uint256 x, uint256 y) internal pure returns (uint256 z) {
        require(y == 0 || (z = x * y) / y == x, "CoreStabilityFeeTreasury/mul-uint-uint-overflow");
    }
    function divide(uint256 x, uint256 y) internal pure returns (uint256 z) {
        require(y > 0, "CoreStabilityFeeTreasury/div-y-null");
        z = x / y;
        require(z <= x, "CoreStabilityFeeTreasury/div-invalid");
    }
    ...
    function modifyParameters(bytes32 parameter, uint256 val) external isAuthorized {
        require(contractEnabled == 1, "CoreStabilityFeeTreasury/not-live");
        if (parameter == "expensesMultiplier") expensesMultiplier = val;
        else if (parameter == "treasuryCapacity") {
          require(val >= minimumFundsRequired, "CoreStabilityFeeTreasury/capacity-lower-than-min-funds");
    ...
    function disableContract() external isAuthorized {
        require(contractEnabled == 1, "CoreStabilityFeeTreasury/already-disabled");
        contractEnabled = 0;
        joinAllCoins();
    ...
    function setTotalAllowance(address account, uint256 rad) external isAuthorized accountNotTreasury(account) {
        require(account != address(0), "CoreStabilityFeeTreasury/null-account");
    ...
    function giveFunds(address account, uint256 rad) external isAuthorized accountNotTreasury(account) {
        ...
        require(safeEngine.coinBalance(address(this)) >= rad, "CoreStabilityFeeTreasury/not-enough-funds");
    ...
    function pullFunds(address dstAccount, address token, uint256 wad) external {
        ...
        require(dstAccount != accountingEngine, "CoreStabilityFeeTreasury/dst-cannot-be-accounting");
        require(wad > 0, "CoreStabilityFeeTreasury/null-transfer-amount");
        require(token == address(systemCoin), "CoreStabilityFeeTreasury/token-unavailable");
        if (allowance[msg.sender].perBlock > 0) {
          require(addition(pulledPerBlock[msg.sender][block.number], multiply(wad, RAY)) <= allowance[msg.sender].perBlock, "CoreStabilityFeeTreasury/per-block-limit-exceeded");
        }
    ...
    function transferSurplusFunds() external {
        require(now >= addition(latestSurplusTransferTime, surplusTransferDelay), "CoreStabilityFeeTreasury/transfer-cooldown-not-passed");
