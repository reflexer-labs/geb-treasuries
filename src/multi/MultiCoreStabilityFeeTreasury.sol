/// MultiCoreStabilityFeeTreasury.sol
...
contract MultiCoreStabilityFeeTreasury {
    // --- Auth ---
    ...
    **/
    modifier isAuthorized(bytes32 coinName) {
        require(authorizedAccounts[coinName][msg.sender] == 1, "MultiCoreStabilityFeeTreasury/account-not-authorized");
        _;
    }
    ...
    modifier coinIsInitialized(bytes32 coinName) {
        require(coinInitialized[coinName] == 1, "MultiCoreStabilityFeeTreasury/coin-not-init");
    ...
    modifier accountNotTreasury(address account) {
        require(account != address(this), "MultiCoreStabilityFeeTreasury/account-cannot-be-treasury");
        _;
    }
    ...
    function addition(uint256 x, uint256 y) internal pure returns (uint256 z) {
        z = x + y;
        require(z >= x, "MultiCoreStabilityFeeTreasury/add-uint-uint-overflow");
    }
    function addition(int256 x, int256 y) internal pure returns (int256 z) {
        z = x + y;
        if (y <= 0) require(z <= x, "MultiCoreStabilityFeeTreasury/add-int-int-underflow");
        if (y  > 0) require(z > x, "MultiCoreStabilityFeeTreasury/add-int-int-overflow");
    }
    function subtract(uint256 x, uint256 y) internal pure returns (uint256 z) {
        require((z = x - y) <= x, "MultiCoreStabilityFeeTreasury/sub-uint-uint-underflow");
    }
    function subtract(int256 x, int256 y) internal pure returns (int256 z) {
        z = x - y;
        require(y <= 0 || z <= x, "MultiCoreStabilityFeeTreasury/sub-int-int-underflow");
        require(y >= 0 || z >= x, "MultiCoreStabilityFeeTreasury/sub-int-int-overflow");
    }
    function multiply(uint256 x, uint256 y) internal pure returns (uint256 z) {
        require(y == 0 || (z = x * y) / y == x, "MultiCoreStabilityFeeTreasury/mul-uint-uint-overflow");
    }
    function divide(uint256 x, uint256 y) internal pure returns (uint256 z) {
        require(y > 0, "MultiCoreStabilityFeeTreasury/div-y-null");
        z = x / y;
        require(z <= x, "MultiCoreStabilityFeeTreasury/div-invalid");
    }
    ...
    function initializeCoin(
        ...
        require(deployer == msg.sender, "MultiCoreStabilityFeeTreasury/caller-not-deployer");
        require(coinInitialized[coinName] == 0, "MultiCoreStabilityFeeTreasury/already-init");
        require(address(safeEngine) != address(0), "MultiCoreStabilityFeeTreasury/null-safe-engine");

        require(address(CoinJoinLike(coinJoin_).systemCoin()) != address(0), "MultiCoreStabilityFeeTreasury/null-system-coin");
        require(CoinJoinLike(coinJoin_).coinName() == coinName, "MultiCoreStabilityFeeTreasury/invalid-join-coin-name");
        require(extraSurplusReceiver_ != address(0), "MultiCoreStabilityFeeTreasury/null-surplus-receiver");
        require(extraSurplusReceiver_ != address(this), "MultiCoreStabilityFeeTreasury/surplus-receiver-cannot-be-this");
        require(expensesMultiplier_ >= HUNDRED, "MultiCoreStabilityFeeTreasury/invalid-expenses-multiplier");
        require(treasuryCapacity_ >= minimumFundsRequired_, "MultiCoreStabilityFeeTreasury/invalid-treasury-capacity");
        require(surplusTransferDelay_ > 0, "MultiCoreStabilityFeeTreasury/null-surplus-transfer-delay");
    ...
    function modifyParameters(bytes32 parameter, address data) external {
        require(manager == msg.sender, "MultiCoreStabilityFeeTreasury/invalid-manager");
        if (parameter == "manager") {
          manager = data;
        } else if (parameter == "deployer") {
          require(data != address(0), "MultiCoreStabilityFeeTreasury/null-deployer");
          deployer = data;
        }
        else revert("MultiCoreStabilityFeeTreasury/modify-unrecognized-param");
    ...
    function modifyParameters(bytes32 coinName, bytes32 parameter, address addr) external isAuthorized(coinName) {
        require(addr != address(0), "MultiCoreStabilityFeeTreasury/null-addr");
        if (parameter == "extraSurplusReceiver") {
          require(addr != address(this), "MultiCoreStabilityFeeTreasury/accounting-engine-cannot-be-treasury");
    ...
    function modifyParameters(bytes32 coinName, bytes32 parameter, uint256 val) external isAuthorized(coinName) {
        if (parameter == "expensesMultiplier") expensesMultiplier[coinName] = val;
          ...
          require(val >= minimumFundsRequired[coinName], "MultiCoreStabilityFeeTreasury/capacity-lower-than-min-funds");
          treasuryCapacity[coinName] = val;
        }
        else if (parameter == "minimumFundsRequired") {
          require(val <= treasuryCapacity[coinName], "MultiCoreStabilityFeeTreasury/min-funds-higher-than-capacity");
    ...
    */
    ...
    function setTotalAllowance(bytes32 coinName, address account, uint256 rad) external isAuthorized(coinName) accountNotTreasury(account) {
        require(account != address(0), "MultiCoreStabilityFeeTreasury/null-account");
    ...
    function giveFunds(bytes32 coinName, address account, uint256 rad) external isAuthorized(coinName) accountNotTreasury(account) {
        ...
        require(safeEngine.debtBalance(coinName, address(this)) == 0, "MultiCoreStabilityFeeTreasury/outstanding-bad-debt");
        require(safeEngine.coinBalance(coinName, address(this)) >= rad, "MultiCoreStabilityFeeTreasury/not-enough-funds");
    ...
    function pullFunds(bytes32 coinName, address dstAccount, address token, uint256 wad) external {
        if (dstAccount == address(this)) return;
	      require(allowance[coinName][msg.sender].total >= multiply(wad, RAY), "MultiCoreStabilityFeeTreasury/not-allowed");
        require(dstAccount != address(0), "MultiCoreStabilityFeeTreasury/null-dst");
        require(dstAccount != extraSurplusReceiver[coinName], "MultiCoreStabilityFeeTreasury/dst-cannot-be-accounting");
        require(wad > 0, "MultiCoreStabilityFeeTreasury/null-transfer-amount");
        require(token == address(systemCoin[coinName]), "MultiCoreStabilityFeeTreasury/token-unavailable");
        if (allowance[coinName][msg.sender].perBlock > 0) {
          require(addition(pulledPerBlock[coinName][msg.sender][block.number], multiply(wad, RAY)) <= allowance[coinName][msg.sender].perBlock, "MultiCoreStabilityFeeTreasury/per-block-limit-exceeded");
        }
        ...
        require(safeEngine.debtBalance(coinName, address(this)) == 0, "MultiCoreStabilityFeeTreasury/outstanding-bad-debt");
        require(safeEngine.coinBalance(coinName, address(this)) >= multiply(wad, RAY), "MultiCoreStabilityFeeTreasury/not-enough-funds");
        require(safeEngine.coinBalance(coinName, address(this)) >= pullFundsMinThreshold[coinName], "MultiCoreStabilityFeeTreasury/below-pullFunds-min-threshold");
    ...
    function transferSurplusFunds(bytes32 coinName) external coinIsInitialized(coinName) {
        require(now >= addition(latestSurplusTransferTime[coinName], surplusTransferDelay[coinName]), "MultiCoreStabilityFeeTreasury/transfer-cooldown-not-passed");
